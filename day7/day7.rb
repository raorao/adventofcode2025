...
def solve(input)
  ...
  split_count = 0
  timelines_at_position = Hash.new(0)
  timelines_at_position[first_row.index("S")] = 1

  rows.each do |row|
    new_timelines_at_position = Hash.new(0)

    timelines_at_position.each do |position, count|
      case row[position]
      when '.'
        new_timelines_at_position[position] += count
      when '^'
        split_count += 1
        new_timelines_at_position[position - 1] += count
        new_timelines_at_position[position + 1] += count
      end
    end

    timelines_at_position = new_timelines_at_position
  end

  {part1: split_count, part2: timelines_at_position.values.sum}
end
...
puts "example input part1: #{solution[:part1]} part2: #{solution[:part2]}"

# real input
input = File.readlines("day7/input.txt").map(&:chomp)
solution = solve(input)
puts "real input part1: #{solution[:part1]} part2: #{solution[:part2]}"
